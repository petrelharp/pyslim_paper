...
which_slim="slim"
SLiM_model="$PWD/virus_in_host.slim"
...
if [[ ! -f ${outpath}snakefile ]] && [[ -f ${SLiM_seq} ]];then
    ...
    for i in `seq 2 $N`;do
        ...
        recipe=$(echo ${recipe} $(printf -- "-d \"HOST_ID='%s'\"" "${infector_id}"))
...
echo 'creating snakemake file for task took' $(($end - $start)) 'seconds to complete'
