...
of samples in the tree sequence, you can do so using the \tskit function simplify(),
...
%% below is some stuff Shyamie wrote as an intro to this part
Thus far, we have discussed how a forward-in-time simulation is operationalized in an ARG-based
framework with a specific focus on \slim and \tskit software. The individual-based approach
implemented by these packages affords users a great deal of flexibility in modelling complex
scenarios. However, a challenge with forward-in-time simulations
is deciding when to start - that is, at what time point in a population or species' history should we
begin to simulate these dynamics?

A typical \slim simulation begins with an initial set of individuals that are identical to one
another, with no genetic variation or prior genealogical relationships by which to differentiate
them. This is problematic because it assumes a genetically homogenous, clonal starting population,
which is not representative of most real-world populations. Therefore, without first
generating a realistic baseline level of diversity for the starting population, our simulations
would contain less genetic variation than they should have.

There are a few possible approaches for generating this baseline diversity. Staying entirely within the
domain of forward-in-time simulations, one could build in a ``burn-in'' period at the start of the simulation. This
involves running the the forward-in-time procedure process until some equilibrium condition is met,
such as coalescence of all extant nodes, and discarding any events that occurred prior to this point.
However, the burn-in period can be extremely long, and its length can be difficult to
predict ahead of time, especially for complex scenarios.

A hybrid simulation approach tackles this problem by grafting a coalescent phase,
representing the earliest events of the simulation, to the main forward-in-time phase. This
can efficiently generate a reasonably ancestral history for all the individuals that comprise the
start of the forward-in-time phase. There are two main ways to achieve this. The first is by
recapitation

 
Backwards-in-time simulations completely avoid this issue by starting with the final generation of
individuals tracing lineages back until they all naturally coalesce into a single root node.
%% end Shyamie's stuff
